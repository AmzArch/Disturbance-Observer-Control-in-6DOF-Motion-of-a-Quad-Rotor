...
for n = 1 : length(timesteps)
    ...
    fb = -dot((-kx*xerr - kv*verr - m*g*e3 + m*ades - dbarx), R*e3) * e3;
...
legend(ax, "Drone in Ideal Conditions", "With Disturbance", "With DOB", "Desired Path")
hold(ax, "off");
...
if animate
    plot3(axnm, 70*yt', 70*xt', -70*zt', '--k');
...
function [disx, disW] = dis_gen(n)
    ...
    if and(or(n < 4, n > 5), step_disturbance)
        disx = 0;
        disW = 0;
    end
end